...
export const Store = {
    ...
    guardarActividad(actividadData, editId = null) {
        if (editId) {
            ...
            if (index !== -1) {
                // Conservar completada, subtareas y calificación si ya existían
            ...
            this.state.actividades.push({
                ...
                ...actividadData,
                completada: false,
                calificacion: null, // Se inicia vacía
                subtareas: []
            });
        }
        this.save();
    ...
    getActividadById(id) {
        return this.state.actividades.find(a => a.id === id);
    },

    deleteActividad(id) {
        this.state.actividades = this.state.actividades.filter(a => a.id !== id);
        this.save();
    },

    // --- NUEVO LÓGICA DE NOTAS ---
    setCompletada(id, estado) {
        const act = this.getActividadById(id);
        if (act) { act.completada = estado; this.save(); }
    },

    setCalificacion(id, nota) {
        const act = this.getActividadById(id);
        if (act) { act.calificacion = nota; this.save(); }
    },

    getPromedio(materiaNombre) {
        const acts = this.state.actividades.filter(a => 
            a.materia === materiaNombre && 
            a.calificacion !== null && 
            a.calificacion !== undefined && 
            a.calificacion.toString().trim() !== ''
        );
        ...
        acts.forEach(a => {
            ...
            // Convertir letras a escala vigesimal (0-20) para matemáticas
            if (val === 'AD') suma += 20;
            else if (val === 'A') suma += 16;
            else if (val === 'B') suma += 12;
            else if (val === 'C') suma += 8;
            else if (!isNaN(parseFloat(val))) suma += parseFloat(val);
            else return; // Si escriben algo raro, lo ignoramos en la suma
            count++;
        });

        if(count === 0) return null;
        return (suma / count).toFixed(1); // Promedio siempre en formato numérico (Ej: 16.5)
    },
    // ----------------------------------

    addSubtarea(actId, texto) {
        const act = this.getActividadById(actId);
        if (act && texto) { act.subtareas.push({ texto, completada: false }); this.save(); }
    },

    toggleSubtarea(actId, subIndex) {
        const act = this.getActividadById(actId);
        if (act) { act.subtareas[subIndex].completada = !act.subtareas[subIndex].completada; this.save(); }
    },

    deleteSubtarea(actId, subIndex) {
        const act = this.getActividadById(actId);
        if (act) { act.subtareas.splice(subIndex, 1); this.save(); }
    },

    getDashboardActividades(query = "") {
        let filtradas = [...this.state.actividades];
        if (query) {
            filtradas = filtradas.filter(a => a.titulo.toLowerCase().includes(query) || a.materia.toLowerCase().includes(query));
        } else {
            filtradas = filtradas.filter(a => !a.completada);
        }
