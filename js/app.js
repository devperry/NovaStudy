...
const App = {
    ...
    renderPremium() {
        ...
        if (user && user.role === 'premium') {
            ...
            statusBox.innerHTML = `
                ...
                <div style="background: #2563eb; color: white; padding: 15px; border-radius: 8px; text-align:center; font-weight:bold; margin-top:15px; font-size:1.2rem;">
                    💎 3.50 Soles / Semana
                </div>
    ...
    showView(viewId, subjectName = null) {
        if (viewId !== 'form') {
            this.vistaAnterior = (viewId === 'subject') ? subjectName : ((viewId === 'aspecto' || viewId === 'premium') ? viewId : 'dashboard');
        }
        ...
        document.getElementById('fab-add').style.display = (viewId === 'form' || viewId === 'aspecto' || viewId === 'premium' || viewId === 'auth') ? 'none' : 'flex';
        ...
        if (subjectName && viewId === 'subject') {
            ...
            this.renderPremium(); // Dibuja la tarjeta del Club VIP
            this.toggleMenu(true);
        }
    },
    ...
    abrirFormulario(id = null) {
        ...
        if (id) {
            ...
            if (this.vistaAnterior !== 'dashboard' && this.vistaAnterior !== 'aspecto' && this.vistaAnterior !== 'premium') {
    ...
    cerrarFormulario() {
        if (this.vistaAnterior === 'dashboard' || this.vistaAnterior === 'aspecto' || this.vistaAnterior === 'premium') this.showView('dashboard');
    ...
    refrescarVistaActual() {
        if (this.vistaAnterior === 'dashboard' || this.vistaAnterior === 'aspecto' || this.vistaAnterior === 'premium') this.realizarBusqueda();
...
// INICIAR AL CARGAR HTML
document.addEventListener('DOMContentLoaded', () => App.init());

// REGISTRO DE SERVICE WORKER PARA OFFLINE
